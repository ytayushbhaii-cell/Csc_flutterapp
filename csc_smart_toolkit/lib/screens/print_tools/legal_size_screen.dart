...
class LegalSizeScreen extends StatefulWidget {
  const LegalSizeScreen({super.key});

  @override
  State<LegalSizeScreen> createState() => _LegalSizeScreenState();
}

class _LegalSizeScreenState extends State<LegalSizeScreen> {
  String? _imagePath;
  ...
  Future<void> _export() async {
    ...
    try {
      ...
      final file = File('${dir.path}/LegalLayout_$ts.pdf');
      await file.writeAsBytes(pdfBytes);
      await SharePlus.instance.share(ShareParams(files: [XFile(file.path)], subject: 'Legal Size Layout'));
      if (!mounted) return;
      context.read<HistoryProvider>().recordUsage(
        toolId: 'legal-size', toolName: 'Legal Size', category: 'Print Tools',
      );
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Legal Size'), backgroundColor: cs.surface),
      body: ListView(
        ...
        children: [
          ...
          Card(
            child: Padding(
              ...
              child: Row(children: [
                Icon(Icons.description_outlined, color: _kColor, size: 28),
                const SizedBox(width: 12),
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('Legal Paper', style: theme.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w700)),
                  Text('216 × 356 mm  •  Portrait',
